...
@inline function ocean_boundary_state!(m::OceanModel, p::SimpleBox, bctype, x...)
    bctype == 1 && return ocean_boundary_state!(m, CoastlineNoSlip(),           x...)
    bctype == 2 && return ocean_boundary_state!(m, OceanFloorNoSlip(),          x...)
    bctype == 3 && return ocean_boundary_state!(m, OceanSurfaceStressForcing(), x...)
end

@inline ocean_boundary_state!(m::Continuity3dModel, p::SimpleBox, bctype, x...) =
    ocean_boundary_state!(m, CoastlineNoSlip(), x...)

@inline ocean_boundary_state!(m::BarotropicModel, p::SimpleBox, bctype, x...) =
    ocean_boundary_state!(m, CoastlineNoSlip(), x...)
...
function main()
    ...
    grid_3D = DiscontinuousSpectralElementGrid(topl_3D, FloatType=FT, DeviceArray=ArrayType,
                                               polynomialorder=N)
    ...
    dg = OceanDGModel(model, grid_3D, RusanovNumericalFlux(), CentralNumericalFluxSecondOrder(),
                      CentralNumericalFluxGradient())

    barotropic_dg = DGModel(barotropicmodel, grid_2D, RusanovNumericalFlux(),
                            CentralNumericalFluxSecondOrder(), CentralNumericalFluxGradient())
    ...
    if regenRefVals
        ...
        println("# SC ========== Test number 1 reference values and precision match template. ============")
    ...
    if checkRefVals
        include("simple_box_2dt_refvals.jl")
...
function make_callbacks(vtkpath, step, nout, mpicomm, odesolver, dg_slow, model_slow, Q_slow,
                        dg_fast, model_fast, Q_fast)
    isdir(vtkpath) && rm(vtkpath, recursive = true)
...
const cʰ = 1  # typical of ocean internal-wave speed [sqrt(gravity * H)]
const cᶻ = 0
